 ...
 *
 * <p>The <code>OperatorRoutingContext</code> allows the user to attach operators to a relation or a stream entity,
 * if the corresponding relation/stream is included as inputs to the operator. Each operator then executes its own logic
 * and determines which relation/stream to emit the output to. Through the <code>OperatorRoutingContext</code>, the next
 * operators attached to the corresponding output entities (i.e. relations/streams) can then be invoked to continue the
 * stream process task.
 *
 * <p>The <code>OperatorRoutingContext</code> also allows the user to set the system input entities (i.e. relations/streams)
 * that are fed into the operators by the system outside the <code>OperatorRoutingContext</code>, not generated by some
 * operators in the <code>OperatorRoutingContext</code>.
 *
